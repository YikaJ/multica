    ...
    entries: [
      {
        version: "0.3.22",
        date: "2026-06-15",
        title: "列表体验、运行配置和更安全的 Issue 编辑",
        changes: [],
        features: [
          "智能体、自动任务、项目、运行环境、技能和小队现在共用更快的列表体验，行内容、操作、筛选和选择状态都更清楚",
          "命令行现在可以管理工作区仓库，让项目更容易提前准备好本地智能体需要的仓库上下文",
          "Cursor 和 OpenClaw 设置加入了新的运行选项，包括托管的 Cursor MCP 配置，以及连接已有 OpenClaw 网关",
          "编辑评论时，现在可以在保存前看到并控制哪些智能体或小队会开始运行",
        ],
        improvements: [
          "桌面端恢复提示会带上更多页面上下文，用户反馈卡住的窗口时更容易说明发生位置",
          "长 Issue 和收件箱视图在跳转后返回时，会更稳定地保留滚动位置和评论锚点",
          "Cursor 的 Composer、缓存输入和新版 Cursor 智能体输出现在会显示更清楚的用量和计费信息",
        ],
        fixes: [
          "Issue 附件、正文图片和文件卡片在网页端、桌面端、移动端以及令牌访问场景下会更稳定地保持可访问",
          "编辑器和只读 Issue 内容现在会更准确地处理金额里的美元符号和邮箱链接",
          "桌面端 Cmd+W 现在会先关闭当前标签页，无法关闭标签页时再关闭窗口",
          "自托管 Docker Compose 上传和默认配置不容易再失败，也会更早发现缺失的配置值",
          "智能体任务的运行令牌无效时，现在会更安全地拒绝继续执行",
        ],
      },
      {
        version: "0.3.21",
