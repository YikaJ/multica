    ...
    entries: [
      {
        version: "0.3.35",
        date: "2026-07-02",
        title: "Issue 视图更快、新增「显示子 Issue」开关，智能体 CLI 更安全",
        changes: [],
        features: [
          "Issue 视图新增「显示子 Issue」开关，在看板、列表、泳道、甘特图里都可以一键聚焦到上层 Issue。",
          "手动新建 Issue 弹窗把标签选择器摆到显眼位置，截止日期则收进 ⋯ 折叠菜单。",
          "技能详情页可以在「被 N 个智能体使用」侧栏里一次把技能挂给多个智能体，选择器还带智能体搜索。",
          "自托管：S3 兼容存储新增 path-style 寻址开关，兼容那些必须走 path-style 的对象存储服务。",
        ],
        improvements: [
          "看板、列表、泳道、甘特图现在共用同一套查询与缓存路径：成员／智能体标签页的总数和「加载更多」都精确一致，跨项目移动 Issue 会按列表精细对账，视窗外的字段变化不再触发一连串重新拉取。慢网络下，共用的刷新指示器会平滑淡入，排序和筛选切换不再看起来像卡住。",
          "对话实时时间线不再在每一条流式任务消息到来时重新挂载，长时间的智能体运行不会再因为反复解析 Markdown 而卡住渲染。",
        ],
        fixes: [
          "同一小队里子 Issue 完成时，父 Issue 所属小队的 Leader 现在会被叫醒——小队自己拆解父 Issue 时，父 Issue 不再滞留在处理中。",
          "受守护进程托管的智能体 CLI 一旦丢失任务 token，会直接失败而不是回退到本地用户的 PAT，智能体的写操作不再冒充成工作区拥有者。（社区反馈）",
          "Slack 告警卡片（Grafana、Incoming Webhook 等）正文只在 Attachments / Blocks / rich_text 里的情况，聊天智能体读取历史时会把这些内容找回来，@ 智能体处理告警卡片重新可用。",
          "内联 base64 图片（二维码、截图、图表）现在能在 Markdown 与只读 Issue 评论里正常渲染，不再显示为破损图片。",
          "评论跨 Issue 移动后，附件链接依然能正确定位到原始附件。",
          "Antigravity 运行「输出为空但已完成」的场景下，找回的最终回复会补进运行时间线，不再留下一段空白。",
          "附件预览接受的文本文件后缀与前端展示的清单对齐，此前被误拦的类型现在可以正常预览。",
          "Mermaid 语法出错时不再把内置的错误图注入到页面里，取而代之的是紧凑的错误提示。",
          "本地技能重新对 ACP 类型运行时可见。",
          "应用内反馈提交前会校验服务端响应并透传错误类型，桌面端路由渲染错误会以 Bug 反馈形式提交。",
          "守护进程调用仓库缓存的 git 命令带上超时，卡死的 git 调用不再拖住运行时。（社区反馈）",
          "集群部署下，运行时的 pending 键会保留在同一个 Redis slot 里。（社区反馈）",
          "Web Docker 镜像现在从 packageManager 字段推导 pnpm 版本，与仓库锁定的 pnpm 保持一致。（社区反馈）",
        ],
      },
      {
        version: "0.3.34",
