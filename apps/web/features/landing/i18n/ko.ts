      ...
      entries: [
        {
          version: "0.3.39",
          date: "2026-07-06",
          title: "Qoder·TRAE CLI를 커스텀 런타임 베이스로, 셀프호스트 검색 안정화, 스쿼드·장기 태스크 신뢰성 수정",
          changes: [],
          features: [
            "Qoder가 커스텀 런타임 프로필의 정식 베이스가 되었습니다. Qoder CN(`qoderclicn`) 사용자는 이제 Kiro 백엔드를 경유하지 않고 Qoder 위에 직접 커스텀 런타임을 구성할 수 있습니다.",
            "ByteDance TRAE CLI(`traecli`)도 커스텀 런타임 프로필의 베이스로 편입되었습니다. TRAE 기반 커스텀 프로필이 엔드투엔드로 수락되며, 런타임 베이스 선택기에도 표시됩니다.",
          ],
          improvements: [
            "공개 문서와 런타임 프로바이더 페이지를 Qoder와 TRAE CLI를 포함한 현재 14종 런타임 목록에 맞춰 갱신했고, 모든 언어 사이트에 반영했습니다.",
          ],
          fixes: [
            "프라이빗 리더 스쿼드: 에이전트나 시스템이 하위 Issue를 종료해도 상위 Issue가 속한 스쿼드의 리더가 정확히 깨어나 다단계 스쿼드 파이프라인이 첫 단계에서 멈추지 않습니다.",
            "단계형 스쿼드의 '하위 완료' 상위 코멘트가 다음 단계가 아직 만들어지지 않았을 때 현재 단계를 '최종 단계'라고 단정하지 않고, '다음 단계 생성'과 '마무리' 두 가능성을 모두 제시해 리더에게 판단을 돌려줍니다.",
            "서버는 벽 시계만으로 정상 실행 중인 장기 태스크를 실패 처리하지 않습니다. 실행 중 태스크 스위퍼는 데몬 런타임이 오프라인이거나 하트비트가 만료된 경우에만 running 행을 실패시키므로, 수 시간 걸리는 리서치·훈련 태스크가 중간에 회수되지 않고 끝까지 실행됩니다.",
            "pg_bigm이 없는 Postgres 이미지에서 멈추던 셀프호스트 검색이, 표준 Postgres에 기본 제공되는 pg_trgm GIN 인덱스로 폴백되고 쿼리 단위 타임아웃 가드까지 더해져, 큰 워크스페이스에서도 첫 검색이 응답을 돌려줍니다.",
            "Issue·코멘트 에디터에 Java 스택트레이스나 길게 조작된 문자열을 붙여 넣어도 탭이 멈추지 않습니다. mention·슬래시 커맨드·파일 카드 라벨 파서를 선형 시간으로 다시 작성했습니다.",
            "Antigravity 런타임이 Claude와 함께 설치된 호스트에서 Claude 전용 settings 플래그가 전달되어도 실패하지 않습니다.",
            "Windows용 Browser MCP 설정을 Windows 셸에서 정확히 해석되는 경로·이스케이프로 생성해, Windows 에이전트에서도 Browser MCP가 실행됩니다.",
            "Codex MCP allowlist 설정이 다시 정상적으로 렌더링되어 MCP allowlist가 무효화되지 않습니다.",
            "Pi(`pi-coding-agent`) 태스크의 최종 출력에 중간 단계 텍스트가 포함되지 않고 최종 결과만 남습니다.",
            "오토파일럿은 실행이 예상보다 오래 걸릴 때 같은 Issue를 연속으로 두 번 디스패치하지 않습니다.",
            "본문에서 Issue 키를 스치듯 언급하기만 한 PR(예: 'Related to MUL-…')은 Issue의 PR 목록에서 감춰지고, 제목 프리픽스·브랜치·클로징 키워드로 실제로 그 Issue를 대상으로 삼는 PR만 표시됩니다.",
            "Issue 액션 메뉴에서 중첩되어 있던 'More' 서브메뉴가 Network 아이콘의 'Relations(관계)'로 이름을 바꾸고, Status·Priority·Start date·Due date 같은 형제 서브메뉴의 명사형 라벨 패턴과 통일되었습니다.",
            "채팅 컴포저, Issue 생성 모달, 빠른 생성, Issue 본문, 피드백 모달 등 모든 첨부 업로드 버튼에서 OS 파일 대화상자로 여러 파일을 한 번에 선택할 수 있습니다.",
          ],
        },
        {
          version: "0.3.36",
