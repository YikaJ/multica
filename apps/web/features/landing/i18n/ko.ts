      ...
      entries: [
        {
          version: "0.3.35",
          date: "2026-07-02",
          title: "더 빠른 Issue 뷰, 하위 Issue 표시 토글, 그리고 더 안전한 에이전트 CLI 쓰기",
          changes: [],
          features: [
            "Issue 뷰에 '하위 Issue 표시' 디스플레이 토글이 추가되어, 보드·리스트·스윔레인·간트 어느 뷰에서든 상위 Issue 에만 집중할 수 있습니다.",
            "Issue 수동 생성 대화상자에 라벨 선택기가 눈에 띄는 자리에 노출되고, 마감일은 ⋯ 오버플로 메뉴로 이동했습니다.",
            "각 스킬 상세 페이지의 '사용 중인 에이전트 N' 사이드바에서 한 번에 여러 에이전트에 스킬을 부착할 수 있고, 선택기에 에이전트 검색이 추가되었습니다.",
            "셀프호스트: S3 호환 오브젝트 스토리지에 path-style 어드레싱 옵션이 추가되어, path-style 이 필요한 프로바이더도 사용할 수 있습니다.",
          ],
          improvements: [
            "보드·리스트·스윔레인·간트가 하나의 쿼리와 캐시 경로를 공유하도록 정리되었습니다. 멤버·에이전트 탭 총계와 '더 보기' 페이지네이션이 정확해지고, 프로젝트 간 이동은 리스트 단위로 정확히 대사되며, 화면 밖 필드 변경은 더 이상 대량 리페치를 유발하지 않습니다. 느린 네트워크에서는 공용 새로고침 인디케이터가 부드럽게 페이드인되어 정렬·필터 전환이 멈춘 것처럼 보이지 않습니다.",
            "채팅 라이브 타임라인이 매 스트리밍 태스크 메시지마다 다시 마운트되지 않도록 수정되어, 오래 실행되는 에이전트 태스크에서도 Markdown 재파싱으로 렌더러가 얼어붙지 않습니다.",
          ],
          fixes: [
            "같은 스쿼드 내에서 하위 Issue 가 완료될 때, 부모 Issue 의 스쿼드 리더가 이제 확실히 깨어납니다. 스쿼드가 자체 부모 Issue 를 분해하는 경우에도 부모 Issue 가 in_progress 에서 멈추지 않습니다.",
            "데몬 관리하의 에이전트 CLI 가 태스크 토큰을 잃으면, 사용자의 개인 PAT 로 조용히 폴백하지 않고 즉시 실패합니다. 에이전트의 쓰기 작업이 워크스페이스 소유자의 이름으로 남는 일이 없어졌습니다. (커뮤니티 보고)",
            "본문이 첨부·Block Kit·rich_text 에만 있는 Slack 알림 카드(Grafana, 인커밍 웹훅 등)의 텍스트를 채팅 에이전트가 히스토리를 읽을 때 복원해, 알림 카드 아래에서 @에이전트 멘션이 다시 동작합니다.",
            "인라인 base64 이미지(QR 코드, 스크린샷, 차트)가 Markdown 과 읽기 전용 Issue 댓글에서 깨진 이미지 대신 정상적으로 렌더링됩니다.",
            "댓글이 Issue 사이를 이동해도 첨부 링크가 원본 첨부로 정확히 이어집니다.",
            "Antigravity 실행이 '출력 없이 완료' 되어도 복구된 트랜스크립트 응답이 실행 타임라인에 표시되어, 결과가 빈 채로 남지 않습니다.",
            "첨부 파일 프리뷰가 지원하는 텍스트 파일 확장자 화이트리스트가 프런트엔드가 알리는 목록과 일치해, 이전에 잘못 걸러졌던 파일 유형도 정상적으로 열립니다.",
            "Mermaid 렌더러가 문법 오류 시 내장 오류 그래픽을 DOM 에 남기지 않고 짧은 오류 안내 상태로 대체됩니다.",
            "로컬 스킬이 ACP 계열 런타임에도 다시 노출됩니다.",
            "앱 내 피드백 전송이 서버 응답을 검증하고 오류 종류를 그대로 전달해, 데스크탑 라우트 렌더 오류가 버그 피드백으로 정확히 접수됩니다.",
            "데몬이 리포지토리 캐시용 git 명령에 타임아웃을 걸어, 멈춘 git 호출이 런타임을 막지 않도록 했습니다. (커뮤니티 보고)",
            "클러스터 배포에서 런타임의 pending 키가 하나의 Redis 슬롯에 유지됩니다. (커뮤니티 보고)",
            "웹 Docker 이미지가 packageManager 필드에서 pnpm 버전을 도출해, 리포지토리에 고정된 pnpm 과 일치합니다. (커뮤니티 보고)",
          ],
        },
        {
          version: "0.3.34",
