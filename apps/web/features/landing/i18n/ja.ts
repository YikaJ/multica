      ...
      entries: [
        {
          version: "0.3.39",
          date: "2026-07-06",
          title: "Qoder / TRAE CLI をカスタム ランタイム ベースに、セルフホスト検索の高速化、スクワッドと長時間タスクの信頼性修正",
          changes: [],
          features: [
            "Qoder がカスタム ランタイム プロファイルの正式なベースになりました。Qoder CN（`qoderclicn`）ユーザーは Kiro バックエンド経由の迂回なしに、直接 Qoder 上でカスタム ランタイムを構築できます。",
            "ByteDance TRAE CLI（`traecli`）もカスタム ランタイム プロファイルのベースに加わりました。TRAE をベースにしたカスタム プロファイルがエンドツーエンドで受け付けられ、ランタイム ベース選択に表示されます。",
          ],
          improvements: [
            "公開ドキュメントとランタイム プロバイダー ページを、Qoder と TRAE CLI を含む現行 14 種のランタイム ラインナップに更新し、全言語サイトに反映しました。",
          ],
          fixes: [
            "プライベート リーダーのスクワッド：エージェントやシステムがサブ Issue をクローズしたとき、親 Issue の所属スクワッド リーダーが正しく起動されるようになり、複数ステージのスクワッド パイプラインが最初のステージで止まらなくなりました。",
            "ステージ付きスクワッドの「サブ タスク完了」親コメントが、まだ次ステージが作られていない状況で現ステージを「最終ステージ」と断定しなくなりました。コメントは「次ステージを作る」「まとめる」の両方の可能性を提示し、リーダーに判断を戻します。",
            "サーバーは、健全に実行中の長時間タスクを壁時計だけで殺さなくなりました。実行中タスクのスイーパーは、デーモン ランタイムがオフラインまたはハートビート切れになった場合にのみ「実行中」の行を失敗扱いにするため、数時間かかる研究・訓練タスクが途中で回収されずに完走できます。",
            "pg_bigm が入っていない Postgres イメージでフリーズしていたセルフホストの検索が、標準 Postgres に同梱される pg_trgm GIN インデックスへのフォールバックと、クエリ単位のタイムアウト バックストップにより、大規模ワークスペースでも回転し続けずに結果を返すようになりました。",
            "Issue やコメント エディターに Java スタックトレースや細工された長い文字列を貼り付けてもタブがフリーズしなくなりました。mention・スラッシュ コマンド・ファイル カード ラベルのパーサーを線形時間で動くように書き直しています。",
            "Antigravity ランタイムは、Claude と同居しているホストで Claude 固有の settings フラグが渡っても落ちなくなりました。",
            "Windows 向け Browser MCP の設定を、Windows シェルで正しく解釈されるパス・クオートで生成するようにし、Windows エージェント上でも Browser MCP が起動します。",
            "Codex MCP の allowlist 設定が再び正しくレンダリングされ、MCP allowlist が失効しなくなりました。",
            "Pi（`pi-coding-agent`）タスクの最終出力から中間ステップの文字列が取り除かれ、最終回答のみが残るようになりました。",
            "オートパイロットは、実行が想定より長引いたときに同じ Issue を続けて 2 回ディスパッチしなくなりました。",
            "本文で Issue キーに触れているだけの PR（例:「Related to MUL-…」）は Issue の PR リストから外れ、タイトル プレフィックス／ブランチ／クローズ キーワードで実際にその Issue を対象にしている PR だけが表示されるようになりました。",
            "Issue アクション メニューの入れ子だった「More」サブメニューを、Network アイコン付きの「Relations（関係）」に改名し、Status／Priority／Start date／Due date と同じ意味ラベルのパターンに揃えました。",
            "アプリ内すべての添付アップロード ボタン（チャット コンポーザー、Issue 作成モーダル、クイック作成、Issue 説明、フィードバック モーダル）で、OS のファイル ダイアログから複数ファイル選択できるようになりました。",
          ],
        },
        {
          version: "0.3.36",
