      ...
      entries: [
        {
          version: "0.3.35",
          date: "2026-07-02",
          title: "より軽快な Issue ビュー、サブ Issue 表示トグル、より安全なエージェント CLI 書き込み",
          changes: [],
          features: [
            "Issue ビューに「サブ Issue を表示」のトグルが追加され、ボード／リスト／スイムレーン／ガントのどのビューでも親 Issue に集中して閲覧できます。",
            "手動の Issue 作成ダイアログにラベル ピッカーが目立つ位置に配置され、期限日は ⋯ オーバーフロー メニューに移動しました。",
            "各スキルの詳細ページの「N 個のエージェントで使用中」サイドバーから、複数のエージェントに一括でスキルを付与でき、ピッカーにエージェント検索も加わりました。",
            "セルフホスト：S3 互換のオブジェクト ストレージに path-style アドレッシングのフラグが加わり、path-style が必須のプロバイダーでも利用できます。",
          ],
          improvements: [
            "ボード、リスト、スイムレーン、ガントが 1 つのクエリとキャッシュ経路を共有するように整理されました。メンバー／エージェント タブの合計と「もっと見る」のページングが正確になり、プロジェクト間の移動はリスト単位で厳密に整合し、画面外のフィールド変更が大量の再取得を引き起こしません。低速回線ではフェードインする共通の更新インジケーターが、並び替えやフィルターの切り替えで反応の遅れを可視化します。",
            "チャットのライブ タイムラインが、ストリーミング タスク メッセージが届くたびに再マウントされないよう修正され、長時間走るエージェント実行でも Markdown 再解析でレンダラーがフリーズしなくなりました。",
          ],
          fixes: [
            "同じスクワッド内でサブ Issue が完了したときに、親 Issue のスクワッド リーダーが確実に起動されるようになりました。スクワッドが自ら親 Issue を分解するケースでも、親 Issue が in_progress のまま止まりません。",
            "デーモン管理下のエージェント CLI が、タスク トークンを失った場合にユーザーの個人 PAT へ黙って切り替わらずに即座に失敗するようになりました。エージェントの書き込みがワークスペース オーナーの名義で残ることはありません。（コミュニティ報告）",
            "本文が Attachments / Block Kit / rich_text にしか無い Slack のアラート カード（Grafana、Incoming Webhook 等）を、チャット エージェントが履歴を読む際に取り戻し、アラート カード配下での @エージェント メンションが再び機能します。",
            "インラインの base64 画像（QR コード、スクリーンショット、チャート）が Markdown と読み取り専用の Issue コメントで壊れた画像ではなく正常に表示されるようになりました。",
            "コメントが Issue 間を移動しても、添付リンクが元の添付を正しく解決し続けます。",
            "Antigravity の実行が「出力なしで完了」した場合でも、復元されたトランスクリプトが実行タイムラインに現れ、結果が空白のまま残りません。",
            "添付ファイル プレビューの対応テキスト形式の許可リストが、フロントエンドが案内するリストと一致するようになり、以前に誤って除外されていた種類も正しくプレビューされます。",
            "Mermaid レンダラーが構文エラー時に内蔵のエラー グラフィックを DOM に残さず、コンパクトなエラー状態に置き換わるようになりました。",
            "ローカル スキルが ACP 系ランタイムに再び公開されるようになりました。",
            "アプリ内フィードバック送信は、サーバー応答を検証してエラー種別を透過するようになり、デスクトップ ルート レンダラーのエラーがバグ フィードバックとして提出されます。",
            "デーモンがリポジトリ キャッシュの git コマンドにタイムアウトを設定するようになり、停止した git 呼び出しがランタイムを塞がなくなりました。（コミュニティ報告）",
            "クラスタ配置におけるランタイムの pending キーが 1 つの Redis スロットに保持されます。（コミュニティ報告）",
            "Web の Docker イメージが packageManager フィールドから pnpm バージョンを導出し、リポジトリで固定された pnpm と一致します。（コミュニティ報告）",
          ],
        },
        {
          version: "0.3.34",
