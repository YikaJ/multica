...
owns its issue-number namespace, and each workspace has a default team whose key is
the legacy workspace prefix, so a bare `MUL-2759` still routes. Each match writes an
issue ↔ PR link row. This is the link that `multica issue pull-requests` reads back.
