...
func TestGetDaemonWorkspaceRepos_WithDaemonToken(t *testing.T) {
	...
	setHandlerTestWorkspaceRepos(t, []map[string]string{
		{"url": "git@example.com:space/api.git", "description": "API"},
		{"url": "  git@example.com:space/web.git  ", "description": " Web "},
	})
	...
	if resp.Repos[1]["url"] != "git@example.com:space/web.git" {
...
func TestGetDaemonWorkspaceRepos_VersionIgnoresOrderAndDescription(t *testing.T) {
	...
	if _, err := testPool.Exec(context.Background(), `UPDATE workspace SET repos = $1 WHERE id = $2`, []byte(`[{"url":"git@example.com:space/web.git","description":"frontend"},{"url":"git@example.com:space/api.git","description":"backend"}]`), testWorkspaceID); err != nil {
		t.Fatalf("update workspace repos: %v", err)
	}
	...
	if _, err := testPool.Exec(context.Background(), `UPDATE workspace SET repos = $1 WHERE id = $2`, []byte(`[{"url":"git@example.com:space/api.git","description":"backend"},{"url":"git@example.com:space/mobile.git","description":"mobile"}]`), testWorkspaceID); err != nil {
		t.Fatalf("update workspace repos: %v", err)
	}
